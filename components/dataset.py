...
class Batch(object):
    def __init__(self, examples, grammar, vocab, copy=True, cuda=False):
        ...
        self.lay_actions = self.get_layactions()
        self.lay_actions_len= [len(a) for a in self.lay_actions]

        self.max_lay_action_num=max(len(a)for a in self.lay_actions)
    ...
    def get_layactions(self):
        lay_actions=[]
        for e in self.examples:
            lay_actions.append(list(filter(lambda a: isinstance(a.action, ApplyRuleAction) or isinstance(a.action,ReduceAction), e.tgt_actions)))
        lay_actions.sort(key=lambda e:-len(e))    
        return lay_actions
    ...
    def get_frontier_field_idx_lay(self, t):
        ids = []
        for actions in self.lay_actions:
            if t < len(actions):
                ids.append(self.grammar.field2id[actions[t].frontier_field])
                # assert self.grammar.id2field[ids[-1]] == e.tgt_actions[t].frontier_field
            else:
                ids.append(0)
    ...
    def get_frontier_prod_idx_lay(self, t):
        ids = []
        for actions in self.lay_actions:
            if t < len(actions):
                ids.append(self.grammar.prod2id[actions[t].frontier_prod])
                # assert self.grammar.id2prod[ids[-1]] == e.tgt_actions[t].frontier_prod
            else:
                ids.append(0)
    ...
    def get_frontier_field_type_idx_lay(self, t):
        ids = []
        for actions in self.lay_actions:
            if t < len(actions):
                ids.append(self.grammar.type2id[actions[t].frontier_field.type])
                # assert self.grammar.id2type[ids[-1]] == e.tgt_actions[t].frontier_field.type
            else:
                ids.append(0)
    ...
    def init_index_tensors(self):
        ...
        #added by gsh 
        self.apply_lay_rule_idx_matrix=[]
        self.apply_lay_rule_mask=[]
        ...
        for t in range(self.max_lay_action_num):
            lay_app_rule_idx_row=[]
            lay_app_rule_mask_row=[]
          
            for a_id, actions in enumerate(self.lay_actions):
                lay_app_rule_idx = lay_app_rule_mask = 0
                if t < len(actions):
                    action=actions[t].action
                    if isinstance(action, ApplyRuleAction):
                        lay_app_rule_idx = self.grammar.prod2id[action.production]
                        lay_app_rule_mask = 1
                    else:
                        lay_app_rule_idx = len(self.grammar)
                        lay_app_rule_mask = 1
                lay_app_rule_idx_row.append(lay_app_rule_idx)
                lay_app_rule_mask_row.append(lay_app_rule_mask)
            self.apply_lay_rule_idx_matrix.append(lay_app_rule_idx_row)
            self.apply_lay_rule_mask.append(lay_app_rule_mask_row)        

        T = torch.cuda if self.cuda else torch       
        self.apply_lay_rule_idx_matrix = Variable(T.LongTensor(self.apply_lay_rule_idx_matrix))
        self.apply_lay_rule_mask = Variable(T.FloatTensor(self.apply_lay_rule_mask))
    ...
    #added by gsh 
    @cached_property
    def lay_action_var(self):
        return nn_utils.to_lay_input_variable(self.lay_actions, self.grammar,
                                          cuda=self.cuda)
