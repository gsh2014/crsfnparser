...
class Coarse2fineParser(nn.Module):
    ...
    def score(self, examples, return_encode_state=False):
        ...
        #add by gsh 
        lay_dec_init_vec = self.init_decoder_state(lay_last_state,lay_last_cell)  
        #add by gsh
        lay_query_vectors = self.laydecode(batch ,src_encodings , dec_init_vec )
    ...
    def parse(self, src_sent, context=None, beam_size=5, debug=False):
        ...
        src_encodings, (last_state, last_cell) = self.encode(src_sent_var, [len(src_sent)], is_lay_encoder=True)
