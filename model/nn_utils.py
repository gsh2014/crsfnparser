...
def action2id(actions,grammar):
    return [[ grammar.prod2id[w.action.production]if isinstance(w.action,ApplyRuleAction)else len(grammar) for w in a]for a in actions]
...
# FIXME: padding token is 0,for a time 
def to_lay_input_variable(lay_actions, grammar, cuda=False, training=True, append_boundary_sym=False):
    """
    given a list of acitons ,remove the gentoken 
    return a tensor of shape (max_sent_len, batch_size)
    """
  
    action_ids = action2id(lay_actions, grammar)
    sents_t = input_transpose(action_ids, 0)
